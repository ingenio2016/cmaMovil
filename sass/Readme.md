...
folder are mapped to the application's root namespace, 'CoomuceMovil'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "CoomuceMovil"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'CoomuceMovil' in the same way
as `"CoomuceMovil/sass/src"`.
